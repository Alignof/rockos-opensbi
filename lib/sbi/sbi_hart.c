...
static void mstatus_init(struct sbi_scratch* scratch) {
    ...
    /* Enable FPU */
    if (misa_extension('D') || misa_extension('F')) mstatus_val |= MSTATUS_FS;

    /* Enable Vector context */
    if (misa_extension('V')) mstatus_val |= MSTATUS_VS;

    csr_write(CSR_MSTATUS, mstatus_val);

    /* Disable user mode usage of all perf counters except default ones (CY, TM,
     * IR) */
    if (misa_extension('S') &&
    ...
    /* All programmable counters will start running at runtime after S-mode
     * request */
    ...
    for (cidx = 0; cidx <= 28; cidx++) {
        if (!(mhpm_mask & 1 << (cidx + 3))) continue;
#if __riscv_xlen == 32
...
#define __set_menvcfg_ext(__ext, __bits) \
    if (sbi_hart_has_extension(scratch, __ext)) menvcfg_val |= __bits;
        ...
        __set_menvcfg_ext(SBI_HART_EXT_ZICBOZ, ENVCFG_CBZE) __set_menvcfg_ext(
            SBI_HART_EXT_ZICBOM, ENVCFG_CBCFE)
...
#undef __set_menvcfg_ext
        ...
        /*
         * When both Svade and Svadu are present in DT, the default scheme for
         * managing the PTE A/D bits should use Svade. Check Svadu before Svade
         * extension to ensure that the ADUE bit is cleared when the Svade
    ...
    if (misa_extension('S')) csr_write(CSR_SATP, 0);
}
...
static int fp_init(struct sbi_scratch* scratch) {
    ...
    if (!misa_extension('D') && !misa_extension('F')) return 0;

    if (!(csr_read(CSR_MSTATUS) & MSTATUS_FS)) return SBI_EINVAL;

#ifdef __riscv_flen
    for (i = 0; i < 32; i++) init_fp_reg(i);
 ...
 */
static unsigned int sbi_hart_get_smepmp_flags(
    struct sbi_scratch* scratch, struct sbi_domain* dom,
    ...
    if (SBI_DOMAIN_MEMREGION_IS_SHARED(reg->flags)) {
        ...
        if ((reg->flags & SBI_DOMAIN_MEMREGION_M_ACCESS_MASK) ==
            ...
            sbi_printf(
                "%s: M-mode only regions cannot have"
                "RWX permissions\n",
                __func__);
            return 0;
        }
        ...
        if (reg->flags & SBI_DOMAIN_MEMREGION_M_READABLE) pmp_flags |= PMP_R;
        if (reg->flags & SBI_DOMAIN_MEMREGION_M_WRITABLE) pmp_flags |= PMP_W;
        if (reg->flags & SBI_DOMAIN_MEMREGION_M_EXECUTABLE) pmp_flags |= PMP_X;
    } else if (SBI_DOMAIN_MEMREGION_SU_ONLY_ACCESS(reg->flags)) {
        if (reg->flags & SBI_DOMAIN_MEMREGION_SU_READABLE) pmp_flags |= PMP_R;
        if (reg->flags & SBI_DOMAIN_MEMREGION_SU_WRITABLE) pmp_flags |= PMP_W;
        if (reg->flags & SBI_DOMAIN_MEMREGION_SU_EXECUTABLE) pmp_flags |= PMP_X;
    }
...
static int sbi_hart_smepmp_set(struct sbi_scratch* scratch,
                               ...
                               struct sbi_domain_memregion* reg,
                               unsigned int pmp_idx, unsigned int pmp_flags,
    ...
    if (pmp_log2gran <= reg->order && pmp_addr < pmp_addr_max) {
        ...
        sbi_printf(
            "Can not configure pmp for domain %s because"
...
static int sbi_hart_smepmp_configure(struct sbi_scratch* scratch,
    ...
    sbi_domain_for_each_memregion(dom, reg) {
        /* Skip reserved entry */
        if (pmp_idx == SBI_SMEPMP_RESV_ENTRY) pmp_idx++;
        if (pmp_count <= pmp_idx) break;
...
int sbi_hart_map_saddr(unsigned long addr, unsigned long size) {
    ...
    if (!sbi_hart_has_extension(scratch, SBI_HART_EXT_SMEPMP)) return SBI_OK;

    if (is_pmp_entry_mapped(SBI_SMEPMP_RESV_ENTRY)) return SBI_ENOSPC;

    for (order = MAX(sbi_hart_pmp_log2gran(scratch), log2roundup(size));
         order <= __riscv_xlen; order++) {
        if (order < __riscv_xlen) {
            base = addr & ~((1UL << order) - 1UL);
            if ((base <= addr) && (addr < (base + (1UL << order))) &&
                (base <= (addr + size - 1UL)) &&
...
int sbi_hart_pmp_configure(struct sbi_scratch* scratch) {
    ...
    if (sbi_hart_has_extension(scratch, SBI_HART_EXT_SMEPMP))
        rc = sbi_hart_smepmp_configure(scratch, pmp_count, pmp_log2gran,
                                       pmp_addr_max);
    else
        rc = sbi_hart_oldpmp_configure(scratch, pmp_count, pmp_log2gran,
                                       pmp_addr_max);
    ...
    if (misa_extension('S')) {
         ...
         */
        if (misa_extension('H')) __sbi_hfence_gvma_all();
    }
...
static inline void __sbi_hart_update_extension(
    struct sbi_hart_features* hfeatures, enum sbi_hart_extensions ext,
    bool enable) {
    if (enable)
...
void sbi_hart_update_extension(struct sbi_scratch* scratch,
                               enum sbi_hart_extensions ext, bool enable) {
    struct sbi_hart_features* hfeatures =
        sbi_scratch_offset_ptr(scratch, hart_features_offset);
...
void sbi_hart_get_extensions_str(struct sbi_scratch* scratch,
    ...
    if (!extensions_str || nestr <= 0) return;
    sbi_memset(extensions_str, 0, nestr);

    for_each_set_bit(ext, hfeatures->extensions, SBI_HART_EXT_MAX) {
        sbi_snprintf(extensions_str + offset, nestr - offset, "%s,",
                     sbi_hart_ext[ext].name);
...
static int hart_detect_features(struct sbi_scratch* scratch) {
    ...
    if (hfeatures->detected) return 0;
...
#define __check_hpm_csr_2(__csr, __mask) \
    __check_hpm_csr(__csr + 0, __mask) __check_hpm_csr(__csr + 1, __mask)
#define __check_hpm_csr_4(__csr, __mask) \
    __check_hpm_csr_2(__csr + 0, __mask) __check_hpm_csr_2(__csr + 2, __mask)
#define __check_hpm_csr_8(__csr, __mask) \
    __check_hpm_csr_4(__csr + 0, __mask) __check_hpm_csr_4(__csr + 4, __mask)
#define __check_hpm_csr_16(__csr, __mask) \
    __check_hpm_csr_8(__csr + 0, __mask) __check_hpm_csr_8(__csr + 8, __mask)
...
#define __check_priv(__csr, __base_priv, __priv)                   \
    ...
    __check_priv(CSR_MCOUNTEREN, SBI_HART_PRIV_VER_UNKNOWN,
                 SBI_HART_PRIV_VER_1_10);
    /* Detect if hart supports Priv v1.11 */
    __check_priv(CSR_MCOUNTINHIBIT, SBI_HART_PRIV_VER_1_10,
                 SBI_HART_PRIV_VER_1_11);
    /* Detect if hart supports Priv v1.12 */
    __check_priv(CSR_MENVCFG, SBI_HART_PRIV_VER_1_11, SBI_HART_PRIV_VER_1_12);
...
#define __check_ext_csr(__base_priv, __csr, __ext)                            \
    if (hfeatures->priv_version >= __base_priv) {                             \
        ...
        if (!trap.cause) __sbi_hart_update_extension(hfeatures, __ext, true); \
    }

    /* Counter overflow/filtering is not useful without mcounter/inhibit */
    /* Detect if hart supports sscofpmf */
    __check_ext_csr(SBI_HART_PRIV_VER_1_11, CSR_SCOUNTOVF,
                    SBI_HART_EXT_SSCOFPMF);
    /* Detect if hart supports time CSR */
    __check_ext_csr(SBI_HART_PRIV_VER_UNKNOWN, CSR_TIME, SBI_HART_EXT_ZICNTR);
    /* Detect if hart has AIA local interrupt CSRs */
    __check_ext_csr(SBI_HART_PRIV_VER_UNKNOWN, CSR_MTOPI, SBI_HART_EXT_SMAIA);
    /* Detect if hart supports stimecmp CSR(Sstc extension) */
    __check_ext_csr(SBI_HART_PRIV_VER_1_12, CSR_STIMECMP, SBI_HART_EXT_SSTC);
    /* Detect if hart supports mstateen CSRs */
    __check_ext_csr(SBI_HART_PRIV_VER_1_12, CSR_MSTATEEN0,
                    SBI_HART_EXT_SMSTATEEN);
    /* Detect if hart supports smcntrpmf */
    __check_ext_csr(SBI_HART_PRIV_VER_1_12, CSR_MCYCLECFG,
                    SBI_HART_EXT_SMCNTRPMF);
    /* Detect if hart support sdtrig (debug triggers) */
    __check_ext_csr(SBI_HART_PRIV_VER_UNKNOWN, CSR_TSELECT,
                    SBI_HART_EXT_SDTRIG);
...
#undef __check_ext_csr
    ...
    rc = sbi_platform_extensions_init(sbi_platform_thishart_ptr(), hfeatures);
    if (rc) return rc;

    /* Zicntr should only be detected using traps */
    __sbi_hart_update_extension(hfeatures, SBI_HART_EXT_ZICNTR, has_zicntr);

    /* Extensions implied by other extensions and features */
    if (hfeatures->mhpm_mask)
        __sbi_hart_update_extension(hfeatures, SBI_HART_EXT_ZIHPM, true);
...
int sbi_hart_init(struct sbi_scratch* scratch, bool cold_boot) {
    ...
    if (cold_boot) {
        ...
        hart_features_offset =
            sbi_scratch_alloc_offset(sizeof(struct sbi_hart_features));
        if (!hart_features_offset) return SBI_ENOMEM;
    }
...
void __attribute__((noreturn)) sbi_hart_switch_mode(unsigned long arg0,
    ...
    switch (next_mode) {
        case PRV_M:
            break;
        case PRV_S:
            if (!misa_extension('S')) sbi_hart_hang();
            break;
        case PRV_U:
            if (!misa_extension('U')) sbi_hart_hang();
            break;
        default:
            sbi_hart_hang();
    }
...
#else
    if (misa_extension('H')) val = INSERT_FIELD(val, MSTATUS_MPV, next_virt);
#endif
